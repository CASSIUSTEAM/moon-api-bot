...
run = (msg, matches) ->
	if matches[1] == 'track' and matches[2]
		return track msg, matches[2]
	elseif matches[1] == 'music' and matches[2]
		return track msg, matches[2], true
	elseif matches[1] == 'artist' and matches[2]
		return artist msg, matches[2]
	elseif matches[1] == 'album' and matches[2]
		return album msg, matches[2]
	elseif matches[1] == 'playlist' and matches[2]
		return playlist msg, matches[2]
	elseif matches[1] == "music"
		return "Use `/help Music` to view the commands"

return {
	description: "*Music plugin !*"
	usage: [[
`/music [track name(for search)]`
will send you preview of that song (only 30 sec)
`/track [track name(for search)]`
`/album [album name(for search)]`
`/artist [artist name(for search)]`
`/playlist [playlist name(for search)]`
